...
提示词中只有技能索引（轻量），完整技能内容按需加载——与 [[progressive-disclosure]] 和 [[tool-search]] 的设计理念完全一致。

## 与其他资料的关系

- 与 [[anthropic-code-execution-with-mcp]] 高度呼应：Anthropic 文章中的 Skills 概念（可复用代码单元）在 Hermes 中以"程序记忆层"的形式落地；技能索引+按需加载与 progressive-disclosure 同构
- 与 [[当我们在讨论-harness-的时候我们在讨论什么]] 直接相关：Hermes Agent 正是这篇文章讨论的 Harness 实践主体
- 与 [[harness-knowledge-moat]] 呼应：Hermes 的 MEMORY.md 系统和 Layer 0-T 团队约定类似，都是把稳定的高价值知识固化到 Agent 的常驻上下文中
- 与 [[llm-wiki-pattern]] 呼应：Karpathy 的 LLM Wiki 模式（知识编译一次，持续复用）与 Hermes 的 session_search（历史按需搜索，辅助模型摘要）是同一思路在不同层次的应用
